...
class PatientController extends Controller
{
    ...
    public function createFromUser(Request $request)
    {
        ...
        // التحقق من أن المستخدم له دور Patient
        if (!$user->hasRole('Patient')) {
            return redirect()->route('users.index')
                ->with('error', 'المستخدم المحدد لا يملك دور مريض');
        }

        // التحقق من أن المستخدم ليس لديه سجل مريض بالفعل
        $existingPatient = Patient::where('user_id', $user->id)->first();
        if ($existingPatient) {
            return redirect()->route('patients.edit', $user->id)
                ->with('info', 'هذا المستخدم لديه بيانات مريض بالفعل، يمكنك تعديل البيانات');
        }

        return view('patients::create_from_user', compact('user'));
    }

    /**
     * Store patient details for existing user.
     *
     * @param Request $request
