                    ...
                    <form action="{{ route('doctors.update', $doctor->id) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <select class="form-select @error('category_ids') is-invalid @enderror"
                                ...
                                @foreach($categories as $category)
                                    <option value="{{ $category->id }}"
                                        {{ in_array($category->id, old('category_ids', $doctor->categories->pluck('id')->toArray())) ? 'selected' : '' }}>
                                        {{ $category->name }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            @if($doctor->image)
                                <div class="mb-2">
                                    <img src="{{ asset('storage/' . $doctor->image) }}"
                                         alt="صورة الطبيب" class="img-thumbnail" style="max-width: 200px">
                                </div>
                            @endif
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="governorate"
                                   name="governorate" value="{{ old('governorate', $doctor->governorate) }}">
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="address"
                                   name="address" value="{{ old('address', $doctor->address) }}">
                        </div>

                        <div class="mb-3">
                            <label for="price" class="form-label">سعر الكشف</label>
                            <input type="number" class="form-control" id="price"
                                   name="price" value="{{ old('price', $doctor->price) }}" step="0.01">
                        </div>
